...
if v_u_11 then
	if v_u_3.ControlMode == Enum.ControlMode.MouseLockSwitch then
		if v_u_5.DevComputerMovementMode == Enum.DevComputerMovementMode.ClickToMove then
			v_u_11 = false
		elseif v_u_3.ComputerMovementMode == Enum.ComputerMovementMode.ClickToMove then
			v_u_11 = false
		else
...
function v_u_4.SetIs0irstPerson(_, p13)
	-- upvalues: (ref) v_u_10
	v_u_10 = p13
end
...
if not v_u_2.TouchEnabled then
	local v23 = v_u_5.DevEnableMouseLock
	if v23 then
		if v_u_3.ControlMode == Enum.ControlMode.MouseLockSwitch then
			if v_u_5.DevComputerMovementMode == Enum.DevComputerMovementMode.ClickToMove then
				v23 = false
			elseif v_u_3.ComputerMovementMode == Enum.ComputerMovementMode.ClickToMove then
				...
				v23 = v_u_5.DevComputerMovementMode ~= Enum.DevComputerMovementMode.Scriptable
			end
		else
			v23 = false
		end
