...
function v_u_2.GetPart(p18)
	...
	if #p18.Open == 0 then
		...
		v20("No parts available in the cache! Creating [" .. v21 .. "] new part instance(s) - this amount can be edited by changing the ExpansionSize property of the PartCache instance... (This cache now contains a grand total of " .. v23 .. " parts.)")
...
function v_u_2.ReturnPart(p30, p31)
	-- upvalues: (copy) v_u_2, (copy) v_u_1, (copy) v_u_3
	local v32 = getmetatable(p30) == v_u_2
	assert(v32, ("Cannot statically invoke method \'%s\' - It is an instance method. Call it on an instance of this class created via %s"):format("ReturnPart", "PartCache.new"))
	local v33 = v_u_1.indexOf(p30.InUse, p31)
	if v33 == nil then
		local v34 = error
		local v35 = p31.Name
		local v36 = p31:GetFullName()
		v34("Attempted to return part \"" .. v35 .. "\" (" .. v36 .. ") to the cache, but it\'s not in-use! Did you call this on the wrong part?")
	else
...
function v_u_2.SetCacheParent(p37, p38)
	...
	assert(v39, ("Cannot statically invoke method \'%s\' - It is an instance method. Call it on an instance of this class created via %s"):format("SetCacheParent", "PartCache.new"))
	local v40 = p38:IsDescendantOf(workspace) or p38 == workspace
	assert(v40, "Cache parent is not a descendant of Workspace! Parts should be kept where they will remain in the visible world.")
...
function v_u_2.Expand(p43, p44)
	...
	assert(v45, ("Cannot statically invoke method \'%s\' - It is an instance method. Call it on an instance of this class created via %s"):format("Expand", "PartCache.new"))
	if p44 == nil then
...
function v_u_2.Dispose(p51)
	...
	assert(v52, ("Cannot statically invoke method \'%s\' - It is an instance method. Call it on an instance of this class created via %s"):format("Dispose", "PartCache.new"))
